                    ...
                    <div class="card  d-none">
                        <div class="card-body">
                            <div class="d-flex justify-content-end mb-2">
                                <a href="{{ route('add.coupon') }}" class="btn btn-danger mb-2"><i
                                    ...
                                    <tbody>
                                        @foreach ($data as $key => $item)
                                            <tr>
                                                ...
                                                <td>{{ date('D,Y-m-d', strtotime($item->coupon_validity)) }}</td>
                                                <td><span
                                                        class="badge rounded-pill {{ $item->coupon_validity >= date('Y-m-d') ? ' bg-light-success text-success' : 'bg-light-danger text-danger' }}">{{ $item->coupon_validity >= date('Y-m-d') ? 'Valid' : 'Invalid' }}</span>
                                                </td>
                                                <td>
                                                    <div class="d-flex order-actions">
                                                        <a href="{{ route('add.coupon', $item->id) }}"
